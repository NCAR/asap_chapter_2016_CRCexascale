...
# Mean rate
set label "" at first X_LIMIT*1./FACTOR, second 2803.27 point pt 3 ps 2 lc rgbcolor "#707070";

# Breakpoints
# Unneeded phases separators, nb. breakpoints = 2

# Data accessors to CSV
sampleexcluded(ret,c,r,g,t) = (c eq 'PAPI_TOT_INS' && r eq 'euler_step' && g == 0  && t eq 'e') ? ret : NaN;
sampleunused(ret,c,r,g,t) = (c eq 'PAPI_TOT_INS' && r eq 'euler_step' && g ==  0 && t eq 'un') ? ret : NaN;
sampleused(ret,c,r,g,t) = (c eq 'PAPI_TOT_INS' && r eq 'euler_step' && g == 0 && t eq 'u') ? ret : NaN;
interpolation(ret,c,r,g) = (c eq 'PAPI_TOT_INS' && r eq 'euler_step' && g == 0 ) ? ret : NaN;
slope(ret,c,r,g) = (c eq 'PAPI_TOT_INS' && r eq 'euler_step' && g == 0 ) ? ret : NaN;

# Plot command
plot \
NaN ti 'Mean PAPI_TOT_INS rate' w points pt 3 lc rgbcolor "#707070" ,\
'perfTest.codeblocks.fused.any.any.any.dump.csv' u ($4*FACTOR):(sampleexcluded($6, strcol(5), strcol(2), $3, strcol(1))) ti 'Excluded samples (227777)' axes x2y1 w points pt 3 lc rgbcolor '#A0A0A0',\
'perfTest.codeblocks.fused.any.any.any.dump.csv' u ($4*FACTOR):(sampleunused($6, strcol(5), strcol(2), $3, strcol(1))) ti 'Unused samples (224480)' axes x2y1 w points pt 3 lc rgbcolor '#FFA0A0',\
